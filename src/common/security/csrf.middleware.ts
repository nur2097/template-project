...
export class CsrfMiddleware implements NestMiddleware {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const isEnabled =
      this.configService.get("CSRF_ENABLED") === "true" ||
      process.env.NODE_ENV === "production";

    if (isEnabled) {
      this.logger.warn(
        "CSRF middleware is disabled as csurf package is deprecated. Consider using alternative CSRF protection."
      );
      ...
      this.csrfProtection = (
        ...
        if (referer || origin) {
          const requestHost = referer
            ? new URL(referer).host
            : new URL(origin).host;
          if (requestHost !== host) {
            return res
              .status(403)
              .json({ error: "CSRF protection: Invalid referer/origin" });
          }
        }
